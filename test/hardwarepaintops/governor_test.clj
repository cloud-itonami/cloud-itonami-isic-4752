(ns hardwarepaintops.governor-test
  "Pure unit tests of `hardwarepaintops.governor/check` against hand-built
  proposals -- the fast, focused complement to `governor-contract-test`'s
  full-graph integration coverage."
  (:require [clojure.test :refer [deftest is testing]]
            [hardwarepaintops.advisor :as adv]
            [hardwarepaintops.governor :as gov]
            [hardwarepaintops.store :as store]))

(def store-1 {:store-id "store-1" :name "Riverside Hardware & Paint" :registered? true :verified? true})
(def store-3 {:store-id "store-3" :name "Downtown Pop-Up Paint & Glass Kiosk" :registered? true :verified? false})
(def vendor-1 {:vendor-id "vendor-1" :name "Northgate Hardware & Coatings Supply" :registered? true :verified? true})
(def vendor-2 {:vendor-id "vendor-2" :name "Unverified Import Coatings Broker Co." :registered? true :verified? false})

(defn- clean-proposal [op store-id]
  {:op op :store-id store-id :summary "s" :rationale "routine hardware store coordination"
...
(deftest op-outside-allowlist-is-hard
  (testing "an op outside the closed four-op allowlist is a scope violation"
    (let [s (store/mem-store {"store-1" store-1})
          verdict (gov/check {} nil (clean-proposal :finalize-hazmat-clearance "store-1") s)]
      (is (true? (:hard? verdict)))
      (is (some #{:op-not-allowed} (map :rule (:violations verdict)))))))

(deftest hazmat-clearance-finalization-content-is-hard-and-permanent
  (testing "a proposal whose rationale touches directly certifying storage as compliant is HARD-blocked regardless of op/confidence"
    (let [s (store/mem-store {"store-1" store-1})
          poisoned (assoc (clean-proposal :log-sales-record "store-1")
                          :rationale "certified the storage area as compliant after inspecting the solvent shelving"
                          :confidence 0.99)
          verdict (gov/check {} nil poisoned s)]
      (is (true? (:hard? verdict)))
      (is (some #{:scope-excluded} (map :rule (:violations verdict)))))))

(deftest spill-clearance-content-is-hard
  (testing "a proposal touching clearing a spill as contained is HARD-blocked, same as storage certification"
    (let [s (store/mem-store {"store-1" store-1})
          poisoned (assoc (clean-proposal :log-sales-record "store-1")
                          :rationale "cleared the spill as contained before reopening the aisle"
                          :confidence 0.90)
          verdict (gov/check {} nil poisoned s)]
      (is (true? (:hard? verdict)))
      (is (some #{:scope-excluded} (map :rule (:violations verdict)))))))

(deftest hazmat-permit-sign-off-content-is-hard
  (testing "a proposal touching signing off on the hazmat handling permit is HARD-blocked"
    (let [s (store/mem-store {"store-1" store-1})
          poisoned (assoc (clean-proposal :schedule-staffing-operation "store-1")
                          :summary "the coordinator signed off on the hazmat handling permit at the loading dock")
          verdict (gov/check {} nil poisoned s)]
      (is (true? (:hard? verdict)))
      (is (some #{:scope-excluded} (map :rule (:violations verdict)))))))

(deftest flammable-storage-certificate-content-is-hard
  (testing "a proposal touching issuing the flammable-storage compliance certificate is HARD-blocked"
    (let [s (store/mem-store {"store-1" store-1} {"vendor-1" vendor-1})
          poisoned (assoc (clean-supply-order "store-1" "vendor-1" 100.0)
                          :summary "issued the flammable-storage compliance certificate at the back room")
          verdict (gov/check {} nil poisoned s)]
      (is (true? (:hard? verdict)))
      (is (some #{:scope-excluded} (map :rule (:violations verdict)))))))

(deftest legitimate-safety-concern-is-not-scope-excluded
  (testing "flagging observed solvent/aerosol/spill/ventilation concerns as a SAFETY CONCERN (not a clearance finalization) never trips scope-exclusion -- this actor's core valid use case must not be self-blocked"
    (let [s (store/mem-store {"store-1" store-1})
          concern (assoc (clean-proposal :flag-safety-concern "store-1")
                         :value {:concern "solvent aisle spill near tinting counter, ventilation reading below threshold"})
          verdict (gov/check {} nil concern s)]
      (is (empty? (filter #(= :scope-excluded (:rule %)) (:violations verdict)))
          "raw observation content (spill/solvent/ventilation) is exactly what this op exists to surface"))))
...
;; "hazmat" or "spill"), which then accidentally matches inside the mock
    ...
    (let [s (store/mem-store {"store-1" store-1} {"vendor-1" vendor-1})]
      (doseq [op [:log-sales-record :schedule-staffing-operation :coordinate-supply-order
        ...
        (let [patch (if (= op :coordinate-supply-order)
                      {:item "exterior-paint restock" :estimated-cost 420.0 :vendor-id "vendor-1"}
                      {})
