(ns hardwarepaintops.sim
  ...
  escalates regardless of phase), then a hazmat-handling-safety-concern
  ...
  into the permanently-excluded hazmat-handling-safety-clearance-
  finalization scope."
...
(defn- approve! [actor tid]
  (g/run* actor {:approval {:status :approved :by "hazmat-safety-coordinator-1"}} {:thread-id tid :resume? true}))

(defn -main [& _]
  (let [db (store/seed-db)
        coordinator-phase-1 {:actor-id "coord-1" :actor-role :hardware-store-coordinator :phase 1}
        coordinator-phase-3 {:actor-id "coord-1" :actor-role :hardware-store-coordinator :phase 3}
        actor (op/build db)]
    ...
    (let [r (exec-op actor "t1" {:op :log-sales-record :store-id "store-1"
                                  :patch {:units-sold 42 :tinting-orders 3 :stock-count-delta -45}} coordinator-phase-1)]
      (println r)
      (println "-- human hardware store coordinator approves --")
    ...
    (println (exec-op actor "t2" {:op :log-sales-record :store-id "store-1"
                                  :patch {:units-sold 30 :tinting-orders 1 :stock-count-delta -31}} coordinator-phase-3))
    ...
    (println (exec-op actor "t4" {:op :coordinate-supply-order :store-id "store-1"
                                  :patch {:item "exterior-paint restock" :quantity 200 :estimated-cost 420.0
    ...
    (let [r (exec-op actor "t5" {:op :coordinate-supply-order :store-id "store-1"
                                 :patch {:item "tempered glass sheet order" :quantity 20 :estimated-cost 3200.0
                                         :vendor-id "vendor-1"}} coordinator-phase-3)]
      (println r)
      (println "-- human hardware store coordinator reviews & approves --")
    ...
    (let [r (exec-op actor "t6" {:op :flag-safety-concern :store-id "store-1"
                                 :patch {:concern "solvent aisle spill near tinting counter, ventilation reading below threshold" :confidence 0.92}} coordinator-phase-3)]
      (println r)
      (println "-- human hardware store coordinator reviews & approves --")
    ...
    (println (exec-op actor "t9" {:op :coordinate-supply-order :store-id "store-1"
                                  :patch {:item "import coatings" :quantity 50 :estimated-cost 300.0
    ...
    (println "\n== log-sales-record store-1, advisor drifts into hazmat-handling-safety-clearance-finalization scope -> HARD hold, permanent ==")
