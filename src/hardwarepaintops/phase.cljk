(ns hardwarepaintops.phase
  "Phase 0->3 staged rollout for the ISIC-4752 'Retail sale of hardware,
  paints and glass in specialized stores' operations-coordination actor.
  ...
  rollout milestone still to come. Flagging a hazmat-handling-safety
  concern always needs a human to actually look at it; this actor never
  directly finalizes the clearance itself.
  `hardwarepaintops.governor`'s own `always-escalate-ops` enforces the
  same invariant independently -- two layers, not one, agree on this."
  (:require [hardwarepaintops.governor :as governor]))
...
(defn verdict->disposition
  "Map a HardwarePaintRetailGovernor verdict to a base disposition before
  the phase gate."
  [verdict]
