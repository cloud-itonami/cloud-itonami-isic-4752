(ns hardwarepaintops.operation
  "OperationActor -- one coordination request = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor
  (HardwarePaintRetailAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the HardwarePaintRetailGovernor
  ...
  human store operator/hazmat-safety coordinator. The approver resumes
  with `{:approval {:status :approved}}` (or :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [hardwarepaintops.advisor :as advisor]
            [hardwarepaintops.governor :as governor]
            [hardwarepaintops.phase :as phase]
...
(defn build
  ...
  `hardwarepaintops.store/Store`).
  opts:
    :advisor      -- a `hardwarepaintops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; HardwarePaintRetailAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; HardwarePaintRetailGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
