...
1. Register operator, stores and vendors; independently confirm each
   ...
   seeding `hardwarepaintops.store`.
2. Import existing sales/inventory/tinting-order, staffing and
...
5. Publish a dry-run hazmat-handling-safety-concern flag and audit
   export.
...
governor-bypass resistance, evidence-backed hazmat-handling-safety-
concern reporting and human review for every escalation-gated action.
Certification never covers directly finalizing a hazmat-handling-safety
clearance -- that decision always stays with a qualified human/regulatory
authority outside this actor.
